...
pub fn get_sqrt_ratio_at_tick(tick: i32) -> Result<U256, UniswapV3MathError> {
    ...
    Ok((ratio >> 32) +
        if (ratio % (RUINT_ONE << 32)) == U256::ZERO { U256::ZERO } else { RUINT_ONE })
}
...
pub fn get_tick_at_sqrt_ratio(sqrt_price_x_96: U256) -> Result<i32, UniswapV3MathError> {
    ...
    let mut f = if r > uint!(0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF_U256) { 1_usize.shl(7) } else { 0 };
    ...
    r = if msb >= 128 { ratio.shr(msb - 127) } else { ratio.shl(127 - msb) };

    let mut log_2: I256 = (I256::from(msb) - I256::from(128)).shl(64);

    for i in (51..=63).rev() {
        r = r.overflowing_mul(r).0.shr(127);
        let f = r.shr(128);
        log_2 = log_2.bitor(I256::from_raw(EthersU256(f.shl(i).into_limbs())));
    ...
    r = r.overflowing_mul(r).0.shr(127);
    let f = r.shr(128);
    log_2 = log_2.bitor(I256::from_raw(EthersU256(f.shl(50).into_limbs())));
